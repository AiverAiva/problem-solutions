    ...
    <transition name="fade" mode="out-in">
      ...
      <div v-else class="pt-16 pb-8 flex flex-col items-center">
        ...
        <a
          ...
          href="https://github.com/AiverAiva/problem-solutions/issues/new?assignees=&labels=&template=feature_request.md&title="
          rel="noreferrer"
          target="_blank"
        >
...
export default defineComponent({
  setup() {
    ...
    const data = useAsync(() => {
      return $content("problems", { deep: true })
        .without(["body"])
        .sortBy("title")
        .fetch();
    });
    ...
    watch(
      ...
      async (n) => {
        if (!n) {
          data.value = await $content("problems", { deep: true })
            .without(["body"])
            .sortBy("title")
            .fetch();
          return;
        }
        data.value = await $content("problems")
